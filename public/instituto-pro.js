(() => {
  ...
  ready(() => {
    ...
    link.rel = 'stylesheet';
    link.href = '/instituto-pro.css?v=1';
    ...
    if (nav && !document.querySelector('.iecg-access-strip')) {
      ...
      strip.innerHTML = `
        <div class="iecg-access-inner">
          <span><b>Instituto</b> · Site público para conhecer as formações</span>
          <a href="/portal" aria-label="Abrir Portal Administrativo">Acessar Portal Administrativo ↗</a>
        </div>`;
    ...
    if (courses.length && !document.querySelector('.iecg-course-tools-pro')) {
      ...
      if (target) {
        ...
        tools.innerHTML = `
          <label class="iecg-course-search">
            <span>Pesquisar formação</span>
            <input type="search" placeholder="Digite o nome de um curso..." aria-label="Pesquisar formação">
          </label>
          <div class="iecg-course-count" aria-live="polite"></div>`;
        ...
        const refresh = () => {
          ...
          courses.forEach(card => {
            const show = !q || card.textContent.toLocaleLowerCase('pt-BR').includes(q);
            card.hidden = !show;
            if (show) visible++;
          });
    ...
    document.querySelectorAll('.links a').forEach(a => {
      const text = (a.textContent || '').trim().toLocaleLowerCase('pt-BR');
      if (text.includes('área de acesso') || text.includes('acesso')) {
