(() => {
  ...
  ready(()=>{
    ...
    if(portal){cssOnce('portal-visual-v4.css?v=8','portal-visual');cssOnce('portal-extra.css?v=2','portal-extra');cssOnce('portal-super-polish.css?v=2','portal-super');scriptOnce('portal-quality.js?v=3','portal-quality');scriptOnce('portal-d1.js?v=1','portal-d1');}
    if(instituto)scriptOnce('instituto-pro.js?v=4','instituto-pro');
    const style=document.createElement('style');style.dataset.iecg='app-polish-final';style.textContent=`:focus-visible{outline:3px solid rgba(255,116,0,.42)!important;outline-offset:2px}.iecg-toast{display:flex;flex-direction:column;gap:3px;position:fixed;right:20px;bottom:20px;z-index:99999;max-width:min(420px,calc(100vw - 40px));padding:13px 15px;border-radius:14px;background:#21122b;color:#fff;box-shadow:0 18px 45px rgba(0,0,0,.24);font:600 12px/1.45 Inter,Segoe UI,Arial,sans-serif;animation:iecgIn .2s ease}.iecg-toast strong{color:#ffb265}.iecg-toast span{color:#eee5f3}@keyframes iecgIn{from{transform:translateY(8px);opacity:0}to{transform:none;opacity:1}}@media(prefers-reduced-motion:reduce){.iecg-toast{animation:none}*{scroll-behavior:auto!important}}`;document.head.appendChild(style);
